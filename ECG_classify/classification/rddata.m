function [ANNOTD,ATRTIME,TIME,M] = rddata(number)
%% 读取特定记录以及样本数0~650000
% 返回值ANNOTD返回每一个QRS波所对应的类型的标记
% 返回值ATRTIMR返回每一个QRS波所对应的类型的标记所对应的时间
% 返回值TIME返回时间轴
% 返回值M返回信号幅值

%------ SPECIFY DATA ------------------------------------------------------
%------ 指定数据文件 -------------------------------------------------------
    PATH= 'E:\学习委员\srp\MIT-BIT心律失常数据库'; % 指定数据的储存路径
    HEADERFILE= [num2str(number),'.hea'];        % .hea 格式，头文件，可用记事本打开
    ATRFILE= [num2str(number),'.atr'];                % .atr 格式，属性文件，数据格式为二进制数
    DATAFILE= [num2str(number),'.dat'];             % .dat 格式，ECG 数据
